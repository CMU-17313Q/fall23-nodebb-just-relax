...
function checkSetupFlagEnv() {
    ...
    if (Object.keys(envConfMap).some(key => envKeys.includes(key))) {
        ...
        for (const [evName, evValue] of Object.entries(process.env)) { // Get setup values from env
            if (evName.startsWith('NODEBB_DB_')) {
                setupValue[`${process.env.NODEBB_DB}:${envConfMap[evName]}`] = evValue;
            } else if (evName.startsWith('NODEBB_')) {
                setupValue[envConfMap[evName]] = evValue;
            }
        }
...
async function createAdmin() {
    ...
    questions = [...questions, ...passwordQuestions];
...
async function giveGlobalPrivileges() {
    ...
    await privileges.global.give([...defaultPrivileges,
        'groups:ban',
        'groups:upload:post:file',
        'groups:view:users:info'], 'Global Moderators');
