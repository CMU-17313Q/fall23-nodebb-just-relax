...
usersController.getBannedUsers = async function (request, res) {
	await renderIfAdminOrGlobalMod('users:banned', request, res);
};

usersController.getFlaggedUsers = async function (request, res) {
	await renderIfAdminOrGlobalMod('users:flags', request, res);
};

async function renderIfAdminOrGlobalMod(set, request, res) {
