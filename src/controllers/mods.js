...
modsController.flags.list = async function (request, res) {
    ...
    if (!(isAdminOrGlobalMod || moderatedCids.length > 0)) {
    ...
    filters = filters.reduce((memo, cur) => {
        if (request.query.hasOwnProperty(cur)) {
            if (typeof request.query[cur] === 'string' && request.query[cur].trim() !== '') {
                memo[cur] = request.query[cur].trim();
            } else if (Array.isArray(request.query[cur]) && request.query[cur].length > 0) {
                memo[cur] = request.query[cur];
            }
        }
...
modsController.postQueue = async function (request, res, next) {
    ...
    res.render('post-queue', {
        ...
        posts: postData,
        isAdmin,
        canAccept: isAdmin || isGlobalMod || moderatedCids.length > 0,
        ...categoriesData,
