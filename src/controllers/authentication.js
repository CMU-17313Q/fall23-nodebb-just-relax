...
async function registerAndLoginUser(request, res, userData) {
    ...
    const result = await plugins.hooks.fire('filter:register.shouldQueue', { req: request, res, userData, queue });
    if (result.queue) {
...
authenticationController.registerComplete = async function (request, res) {
    try {
        ...
        const results = await Promise.allSettled(callbacks.map(async (cb) => {
            await cb(request.session.registration, request.body);
        }));
...
function continueLogin(strategy, request, res, next) {
    passport.authenticate(strategy, async (error, userData, info) => {
        if (error) {
            plugins.hooks.fire('action:login.continue', { req: request, strategy, userData, error });
        ...
        if (!userData) {
            ...
            plugins.hooks.fire('action:login.continue', { req: request, strategy, userData, error: new Error(info) });
        ...
        plugins.hooks.fire('action:login.continue', { req: request, strategy, userData, error: null });
