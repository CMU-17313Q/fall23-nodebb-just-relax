...
module.exports = function (module) {
    ...
    async function sortedSetRange(method, key, start, stop, min, max, withScores) {
        if (Array.isArray(key)) {
            ...
            key.forEach(key => batch[method](genParameters(method, key, 0, stop, min, max, true)));
        ...
        const parameters = genParameters(method, key, start, stop, min, max, withScores);
    ...
    function genParameters(method, key, start, stop, min, max, withScores) {
        const parameters = {
