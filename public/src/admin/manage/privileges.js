...
define('admin/manage/privileges', [
    ...
    Privileges.commit = function () {
        ...
        const requests = $.map(tableElement.querySelectorAll('td[data-delta]'), (element) => {
            const { privilege } = element.dataset;
    ...
    Privileges.addUserToPrivilegeTable = function () {
        ...
        modal.on('shown.bs.modal', () => {
            ...
            autocomplete.user(inputElement, (ev, ui) => {
    ...
    Privileges.addGroupToPrivilegeTable = function () {
        ...
        modal.on('shown.bs.modal', () => {
            ...
            autocomplete.group(inputElement, (ev, ui) => {
    ...
    function hightlightRowByDataAttr(attrName, attrValue) {
        if (attrValue) {
            const $element = $('[' + attrName + ']').filter(function () {
    ...
    function addGroupToCategory(group, cb) {
        ...
        const privilegeSet = ajaxify.data.privileges.keys.groups.reduce((memo, cur) => {
            memo[cur] = false;
            return memo;
        }, {});
    ...
    async function addUserToCategory(user, cb) {
        ...
        // Generate data for new row
        const privilegeSet = ajaxify.data.privileges.keys.users.reduce((memo, cur) => {
            memo[cur] = false;
            return memo;
        }, {});
    ...
    function filterPrivileges(ev) {
        ...
        for (const tr of rows) {
            for (const [idx, element] of tr.querySelectorAll('td, th').entries()) {
                ...
                element.classList.toggle('hidden', !(idx >= (startIdx - offset) && idx <= (endIdx - offset)));
            }
        }
