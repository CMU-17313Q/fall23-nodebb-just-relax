...
define('admin/modules/search', ['mousetrap', 'alerts'], (mousetrap, alerts) => {
	...
	function find(dict, term) {
		const html = dict.filter(element => element.translations.toLowerCase().includes(term)).map(parameters => {
	...
	search.init = function () {
		...
		socket.emit('admin.getSearchDict', {}, (error, dict) => {
			if (error) {
				alerts.error(error);
				throw error;
			}
	...
	function setupACPSearch(dict) {
		...
		$('#acp-search').parents('form').on('submit', ev => {
		...
		mousetrap(input[0]).bind(['up', 'down'], (ev, key) => {
			let next;
			if (key === 'up') {
