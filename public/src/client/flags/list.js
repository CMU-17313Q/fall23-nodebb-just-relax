...
define('forum/flags/list', [
    ...
    Flags.enableFilterForm = function () {
        ...
        $filtersElement.find('button[data-target="#more-filters"]').click((ev) => {
            const { textVariant } = ev.target.dataset;
            if (!textVariant) {
                return;
            }

            ev.target.dataset.textVariant = ev.target.textContent;
    ...
    Flags.handleBulkActions = function () {
        document.querySelector('[component="flags/bulk-actions"]').addEventListener('click', (e) => {
            ...
            if (subselector) {
                const { action } = subselector.dataset;
