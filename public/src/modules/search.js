...
define('search', ['translator', 'storage', 'hooks', 'alerts'], (translator, storage, hooks, alerts) => {
	...
	Search.enableQuickSearch = function (options) {
		...
		const searchOptions = {in: config.searchDefaultInQuick || 'titles', ...options.searchOptions};
		const quickSearchResults = options.searchElements.resultEl;
		const {inputEl} = options.searchElements;
		...
		function doSearch() {
			...
			Search.api(options.searchOptions, data => {
				...
				app.parseAndTranslate('partials/quick-search-results', data, html => {
					...
					quickSearchResults.toggleClass('hidden', html.length === 0 || !inputEl.is(':focus'))
		...
		inputEl.off('keyup').on('keyup', utils.debounce(() => {
		...
		inputEl.off('refresh').on('refresh', () => {
			doSearch();
		});
	};
