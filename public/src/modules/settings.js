...
define('settings', ['hooks', 'alerts'], (hooks, alerts) => {
	...
	helper = {
		...
		serializeForm(formElement) {
			...
			formElement.find('input[type="checkbox"]').each((idx, inputElement) => {
			...
			formElement.find('select[multiple]').each((idx, selectElement) => {
	...
	Settings = {
		...
		load(hash, formElement, callback) {
			...
			socket.emit(call || 'admin.settings.get', {
				...
				helper.whenReady(() => {
					$(formElement).find('[data-sorted-list]').each((idx, element) => {
